...
a working join-policy planner and dbt/Dataform emitters (`pipeline_codegen/`) but compiles straight
from its planner to text.
...
2. **Compilers**, one per target, each with its own test suite: `sql` (per dialect, via
   ...
   runner; ports DataPilot's emitters). `spark` is reserved and not implemented until a pilot needs it.
...
4. **Checks are part of the recipe, not afterthoughts.** `join` nodes run a pre-flight
   ...
   aggregate_reconciliation`) as **blocking** or **warning**. A failing blocking gate quarantines
   the candidate output (DataPilot's quarantine pattern, ported) and keeps the previous good version.
...
7. **Incremental** follows ADR-0016: a recipe may declare `incremental {watermark, key,
   ...
   watermark; otherwise it's a full rebuild. Retries use the fixed-cursor rule from that spike.
