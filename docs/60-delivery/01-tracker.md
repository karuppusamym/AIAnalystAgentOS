...
| P4-C08 | Re-run both live e2e scenarios at HEAD, recording run duration; de-duplicate the repeated alert and the ×100 scale variant seen in `e2e-phase3-20260925-064436.md` | P0 | New dated evidence files; alert de-duplication test; register updated | In progress — code part Done (condition-keyed alert de-duplication, ×100 series rejected, durations recorded; `tests/unit/test_monitor_dedupe.py`); live re-runs at HEAD running |
