...
|---|---|---|---|---|
| P4-X01 | Capability manifest schema and registry: discovery from built-ins, directory packs, the `analystos.capabilities` entry points and MCP; validation at load; per-workspace enablement; certification status; hot reload; bound capability versions in the plan hash | P0 | Loading a pack with an unknown skill name fails; a plugin installed with pip appears without code changes to core; reload without restart; run in flight keeps its versions | Partial — manifest contract + registry core (built-ins, legacy bridge, packs, entry points, strict validation, reload snapshot; `tests/unit/test_capability_registry.py`); per-workspace enablement, certification gating and plan-hash binding in progress |
