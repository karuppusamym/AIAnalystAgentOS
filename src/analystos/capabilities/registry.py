...
1. built-in manifests  `src/analystos/capabilities/builtin/**/*.yaml` (playbooks, action skills),
   the built-in analysis methods `src/analystos/methods/*.yaml` (each next to its module, so a method
   is one module plus one manifest) and the agent catalog `config/agents/*.yaml` (Agent manifests;
   old `agent:` files are converted)
...
def load(*, builtin_dir: Path | None = BUILTIN_DIR, packs_dir: Path | None | str = "default", entry_points: bool = True,
         ...
         connectors: bool = True, methods_dir: Path | None = METHODS_DIR, agents_dir: Path | None = None,
         agents: bool | None = None) -> Snapshot:
