...
def _mode_gate(router: ModelRouter, purpose: str, ctx: CallContext | None) -> None:
    ...
    if mode == "off":
        # off = the ladder has no model rung; under auto the service already tried rules first (chain())
...
class JevBackend:
    ...
    def available(self, purpose: str, ctx: CallContext | None) -> bool:
        return self.router.mode(purpose) != "off" and self.router.available(purpose, ctx)
