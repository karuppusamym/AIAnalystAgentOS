...
class JevDecisions:
    ...
    def _call(self, purpose: str, state: dict, questions: dict, ctx: CallContext | None) -> dict | None:
        ...
        if mode == "off" or (mode == "auto" and not self.strict):
            # auto = rules first: direct callers let the rule decide; the DecisionService (strict) has
            # already run its rules and only reaches JEV when they abstained or left a tie
