...
export function RunsPage() {
  ...
  return (
    <div className="page">
      <PageHeader title="Analysis runs" subtitle="Every run is planned, executed by agents under policy, verified, and auditable."
        actions={<Link to={`/w/${wsId}`} className="btn btn-primary">Start analysis</Link>} />
      ...
      {!!runs.data?.length && (
        <div className="table-wrap card">
          <table className="table">
            <thead>
              <tr><th>Objective</th><th>Status</th><th>Autonomy</th><th>Started</th><th>Duration</th><th className="num">Tokens</th><th className="num">Cost</th><th /></tr>
            </thead>
            <tbody>
              {runs.data.map((r) => (
                <tr key={r.id}>
                  ...
                  <td><Link to={`/w/${wsId}/runs/${r.id}/console`} className="btn btn-xs btn-ghost">Console</Link></td>
                </tr>
              ))}
            </tbody>
          </table>
